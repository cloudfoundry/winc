...
var _ = Describe("Flags", func() {
	...
	Context("when passed '--ccg-enabled'", func() {
		var (
			containerId string
			bundlePath  string
			bundleSpec  specs.Spec
		...
		It("creates a container and uses the credential spec path", func() {
			...
			args := []string{"--ccg-enabled", "--credential-spec", file.Name(), "create", containerId, "-b", bundlePath}
		...
		Context("when the credential file does not exist", func() {
			It("returns a helpful error", func() {
				args := []string{"--ccg-enabled", "--credential-spec", "/some/path/to/credential/spec", "create", containerId, "-b", bundlePath}
		...
		Context("when the --credential-spec flag is not provided", func() {
			It("returns a helpful error", func() {
				args := []string{"--ccg-enabled", "create", containerId, "-b", bundlePath}
				stdOut, stdErr, err := helpers.Execute(exec.Command(wincBin, args...))
				Expect(err).To(HaveOccurred(), stdOut.String(), stdErr.String())
				Expect(stdErr.String()).To(ContainSubstring("You must pass the --credential-spec flag when --ccg-enabled is passed"))
				Expect(helpers.ContainerExists(containerId)).To(BeFalse())
			})
		})
	})

	Context("when passed --credential-spec without --ccg-enabled", func() {
		var (
			containerId string
			bundlePath  string
			bundleSpec  specs.Spec
		...
		It("fails with a helpful error", func() {
			...
			stdOut, stdErr, err := helpers.Execute(exec.Command(wincBin, args...))
			Expect(err).To(HaveOccurred(), stdOut.String(), stdErr.String())
			Expect(stdErr.String()).To(ContainSubstring("You must also pass the --ccg-enabled flag in order to specify a credential spec"))
			Expect(helpers.ContainerExists(containerId)).To(BeFalse())
		})
	})
