...
var _ = Describe("Flags", func() {
	...
	FContext("when passed '--log-handle'", func() {
		var (
			logHandle string
			logR      *os.File
			logW      *os.File
		...
		Context("when the winc command logs non error messages", func() {
			...
			Context("when the log handle is not valid", func() {
				It("returns a useful error", func() {
					...
					Expect(err).To(HaveOccurred())
					Expect(err.Error()).To(Equal("a useful error"))
				})
			})

			FContext("when the --debug flag is set", func() {
