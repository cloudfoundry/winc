...
func getAdapterInfoByName(name string, family uint32) (AdapterInfo, error) {
	...
	for aa := (*IP_ADAPTER_ADDRESSES)(unsafe.Pointer(&b[0])); aa != nil; aa = aa.Next {
		...
		if foundName == name {
			return AdapterInfo{Name: foundName, Index: aa.IfIndex, LUID: aa.Luid, CompartmentId: aa.CompartmentId}, nil
		}
	}
...
func getAdapterInfoByIndex(ifIdx uint32, family uint32) (AdapterInfo, error) {
	...
	for aa := (*IP_ADAPTER_ADDRESSES)(unsafe.Pointer(&b[0])); aa != nil; aa = aa.Next {
		if aa.IfIndex == ifIdx {
			...
			return AdapterInfo{Name: name, Index: aa.IfIndex, LUID: aa.Luid, CompartmentId: aa.CompartmentId}, nil
		}
	}
